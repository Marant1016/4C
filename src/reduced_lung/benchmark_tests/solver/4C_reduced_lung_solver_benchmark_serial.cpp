...
namespace
{
  ...
  static void assembly_phases_balanced_airways(benchmark::State& state)
  {
    ...
    for (auto _ : state)
    {
      ...
      phase_start = Clock::now();
      for (const auto& initialize_capacity :
          fixture.assembly_pipeline.tree_linearization_capacity_initializers)
      {
        initialize_capacity(tree_linearization);
      }
      for (const auto& tree_linearization_static_assembler :
          fixture.assembly_pipeline.tree_linearization_static_assemblers)
      {
        tree_linearization_static_assembler.callback(tree_linearization);
      }
      for (const auto& tree_linearization_assembler :
